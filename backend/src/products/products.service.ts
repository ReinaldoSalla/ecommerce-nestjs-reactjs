...
export class ProductsService {
  ...
  public async getProducts(): Promise<Product[]> {
    ...
    console.log(msg);
    var createDocument;
    const documents = await this.productModel.countDocuments({}, (err: Error, numDocuments: number) => {
      if (!err && !numDocuments) createDocument = true; // await cannot be user inside this block
    });
    if (createDocument) await populateDb(this.productModel, "stock");
